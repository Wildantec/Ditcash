...
export default function ModuloVehiculosAdmin({ vehiculosIniciales, vendedores }: ModuloVehiculosProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      Swal.fire({ title: '¡REGISTRO ACTUALIZADO!', icon: 'success', confirmButtonColor: '#001F3F' }).then(() => window.location.reload())
    } else {
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4 pb-2 border-b border-slate-200">
        <div>
          <h1 className="text-lg font-black uppercase italic tracking-tighter flex items-center gap-1.5">
            <Wrench className="text-[#FFB800]" size={20} strokeWidth={2.5} /> Mantenimiento
          </h1>
      ...
      <div className="bg-slate-100 border border-slate-200 rounded-xl p-2.5 mb-4 flex flex-col sm:flex-row items-end gap-3 shadow-inner max-w-3xl">
        <div className="flex flex-col gap-0.5 flex-1 w-full relative" ref={placaDropdownRef}>
          ...
          <button
            ...
            className="w-full px-3 py-1.5 bg-white border border-slate-200 rounded-lg font-bold text-xs text-[#001F3F] flex items-center justify-between uppercase text-left h-[34px]"
          >
        ...
        <div className="flex flex-col gap-0.5 flex-1 w-full">
          ...
          <select
            ...
            className="w-full px-3 py-1.5 text-xs font-bold rounded-lg border border-slate-200 text-[#001F3F] focus:outline-none bg-white uppercase h-[34px]"
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-2.5">
              ...
              <div className="flex flex-col gap-0.5 relative" ref={dropdownRef}>
                ...
                <button
                  ...
                  className="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg font-black text-xs text-[#001F3F] flex items-center justify-between uppercase text-left"
                >
                ...
                {isDropdownOpen && (
                  <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-slate-100 rounded-lg shadow-2xl z-50 overflow-hidden max-h-[120px] overflow-y-auto">
                    <button type="button" onClick={() => { setVendedorId(''); setIsDropdownOpen(false) }} className="w-full px-3 py-2 text-left font-black text-xs text-amber-600 border-b border-slate-50 hover:bg-slate-50">-- DEJAR SIN ASIGNAR --</button>
                    {vendedores.map((v) => (
                      <button
                        key={v.id}
                        type="button"
                        ...
                        className={`w-full px-3 py-2 text-left font-black text-xs border-b border-slate-50 transition-colors uppercase ${
