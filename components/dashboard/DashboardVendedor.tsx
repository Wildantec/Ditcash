...
export default async function DashboardVendedor() {
  ...
  if (!vendedor) {
    async function logoutDeEmergencia() {
      'use server'
      const cookiesActuales = await cookies()
      cookiesActuales.delete('user_id')
      cookiesActuales.delete('user_role')
      redirect('/login')
    }

    return (
      <div className="min-h-screen bg-[#F4F7FA] flex flex-col items-center justify-center p-6 text-[#001F3F]">
        <div className="bg-white p-10 md:p-14 rounded-[2.5rem] shadow-2xl border border-slate-100 text-center max-w-md space-y-6">
          <div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center text-2xl mx-auto shadow-inner">
            ⚠️
          </div>
          <div>
            <h3 className="font-black text-lg uppercase italic tracking-tight">Perfil No Detectado</h3>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mt-2 leading-relaxed">
              Tu ID de usuario corporativo ({userIdRaw}) no se encuentra vinculado a la tabla de vendedores en Ditec.
            </p>
          </div>
          
          <form action={logoutDeEmergencia} className="w-full">
            <button 
              type="submit"
              className="w-full bg-[#001F3F] text-[#FFB800] py-4 rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-[#FFB800] hover:text-[#001F3F] transition-all shadow-md"
            >
    ...
    <div className="min-h-screen bg-[#F4F7FA] p-4 md:p-12 text-[#001F3F] font-sans">
      ...
      <header className="flex flex-col md:flex-row justify-between items-center gap-6 mb-8 md:mb-12">
        <div className="text-center md:text-left">
          ...
          <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em] mt-2">DITCASH | ASESORES DE CAMPO</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="bg-white px-5 py-3 rounded-2xl shadow-xl border border-slate-100 flex items-center gap-4">
            <div className="text-right hidden sm:block">
              <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none">Canal Comercial</p>
              <p className="text-xs font-black text-[#001F3F] uppercase tracking-tighter mt-1">{nombreRealUsuario}</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 mb-8 md:mb-12">
        ...
        <div className="lg:col-span-7 bg-white rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-10 border border-slate-100 shadow-xl flex flex-col justify-between group">
          <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
            ...
            <Link href="/dashboard/premios" className="w-full sm:w-auto">
              ...
              {campanasActivas.length > 0 ? (
                campanasActivas.map((c: any) => (
                  <div key={c.id} className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 flex flex-col md:flex-row items-center justify-between hover:shadow-2xl transition-all border-l-[12px] border-[#001F3F] gap-6">
                    ...
                    <Link href={`/dashboard/campanas/${c.id}`} className="w-full md:w-auto">
              ...
              {topVendedores.map((v: any, index: any) => (
                <div key={v.id} className={`flex justify-between items-center p-4 md:p-6 rounded-[1.5rem] transition-all ${v.id === vendedor.id ? 'bg-[#001F3F] text-white shadow-xl scale-105' : 'bg-slate-50 text-[#001F3F]'}`}>
                  <div className="flex items-center gap-4 md:gap-5">
                    <span className="text-[10px] font-black w-7 h-7 md:w-8 md:h-8 flex items-center justify-center rounded-xl bg-white/20">
                      {index + 1}
                    </span>
