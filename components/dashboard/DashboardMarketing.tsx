...
export default async function DashboardMarketing() {
  const cookieStore = await cookies()
  const userIdRaw = cookieStore.get('user_id')?.value
  
  let nombreUsuario = "GESTIÓN MARKETING"
  if (userIdRaw) {
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-12 text-[#001F3F]">
      <header className="flex flex-col md:flex-row justify-between items-center gap-6 mb-8 md:mb-12">
        <div className="text-center md:text-left">
          <h1 className="text-xl md:text-2xl font-black uppercase italic tracking-tighter leading-none">Panel de Marketing</h1>
          <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em] mt-2">DITCASH | MARKETING CENTRAL</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="bg-white px-5 py-3 rounded-2xl shadow-xl border border-slate-100 flex items-center gap-4">
            <div className="text-right hidden sm:block">
              <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none">Control Comercial</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-10">
        ...
        <Link href="/dashboard/vendedores" className="block group">
          <div className={`p-6 rounded-[2rem] shadow-xl border transition-all duration-500 flex items-center justify-between h-full ${
            pendientesTotal > 0 ? 'bg-orange-50/60 border-orange-200' : 'bg-white border-slate-100'
          }`}>
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-inner transition-all ${
                pendientesTotal > 0 ? 'bg-orange-500 text-white animate-pulse' : 'bg-slate-100 text-[#001F3F]'
              }`}>
