      ...
      {!vendedorSeleccionado ? (
        <>
          <header className="mb-8 pb-4 border-b border-slate-200">
            <h1 className="text-xl md:text-2xl font-black uppercase italic tracking-tighter leading-none flex items-center gap-2">
              <Search size={22} className="text-[#FFB800]" /> Gestión de Vendedores
            </h1>
                    ...
                    <tr key={v.id} className="group hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-10 py-6 text-right">
                        <button 
                          ...
                          className={`text-[10px] font-black uppercase tracking-widest px-6 py-3 rounded-xl transition-all ${tienePendientes ? 'bg-[#FFB800] text-[#001F3F] shadow-lg' : 'bg-slate-100 text-[#001F3F]'}`}
                        >
           ...
           <header className="mb-6 flex flex-col md:flex-row items-center justify-between bg-white p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] shadow-sm border gap-4">
              <div className="flex items-center gap-4 w-full md:w-auto">
                 <button 
                   ...
                   className="w-10 h-10 bg-[#001F3F] text-white rounded-xl flex items-center justify-center hover:bg-[#FFB800] transition-colors shadow-lg font-bold flex justify-center items-center"
                 >
                  ...
                  >
                    ...
                    <div className="absolute top-3 right-3 flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
                        ...
                        <button 
                          ...
                          title="Eliminar"
                          className="bg-white/90 backdrop-blur text-red-600 w-8 h-8 rounded-xl flex items-center justify-center shadow-md transition-colors hover:bg-red-600 hover:text-white"
                        >
                          <Trash2 size={14} />
                        </button>
                    </div>
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    ...
                    {evi.estado === 'pendiente' && (
                      <div className="flex gap-2 mt-auto" onClick={(e) => e.stopPropagation()}>
                        ...
                        >
                          <Check size={11} strokeWidth={3} /> Aprobar
                        </button>
                        <button 
                          onClick={() => handleRevisar(evi.id, false)}
                          className="flex-1 bg-red-50 text-white py-2.5 rounded-xl text-[8px] font-black uppercase shadow-md active:scale-95 transition-all flex justify-center items-center gap-1 hover:bg-red-600"
                        >
