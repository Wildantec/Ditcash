                ...
                <tr key={item.id} className="group hover:bg-slate-50 transition-colors">
                  <td className="px-10 py-8">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[9px] font-bold text-slate-300 uppercase tracking-tighter">{item.fecha_inicio}</span>
                      <span className="text-[9px] font-bold text-[#FFB800]">/</span>
                      <span className="text-[9px] font-bold text-slate-300 uppercase tracking-tighter">{item.fecha_cierre}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-8 py-8 text-center">
                    <span className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border ${item.total_acumulado > 0 ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-slate-50 text-slate-300 border-slate-100'}`}>
                      {item.puesto}
                    </span>
                  </td>
                  ...
                  <td className="px-8 py-8 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span className={`text-[10px] font-black uppercase tracking-widest ${item.estado === 'Activa' ? 'text-green-600' : 'text-red-400'}`}>
                        {item.estado}
                      </span>
                    </div>
                  </td>
            ...
            <div className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl">
              <div>
                ...
                <p className="text-2xl font-black text-[#001F3F] italic tracking-tighter">
                  <span className="text-[#FFB800] text-sm mr-0.5">$</span>{Number(item.total_acumulado).toFixed(2)}
                </p>
              </div>
