...
export default function GestionCanjesAdmin() {
  ...
  async function handleGestion(id: number, aprobado: boolean, nombrePremio: string, nombreVendedor: string) {
    ...
    const confirm = await Swal.fire({
      title: `<span style="font-size:16px; font-weight:900; text-transform:uppercase; color:#001F3F; letter-spacing:0.05em;">¿${accion} CANJE?</span>`,
      text: `${aprobado ? 'Confirmarás' : 'Anularás'} la entrega de ${nombrePremio.toUpperCase()} para ${nombreVendedor.toUpperCase()}.`,
    ...
    if (confirm.isConfirmed) {
      Swal.fire({ 
        title: '<span style="font-size:14px; font-weight:bold; uppercase; color:#001F3F;">PROCESANDO TRANSACCIÓN...</span>', 
      ...
      if (res.success) {
        Swal.fire({
          title: '<span style="font-size:16px; font-weight:bold; text-transform:uppercase; color:#001F3F;">¡PROCESADO!</span>',
          text: `Canje ${aprobado ? 'aprobado' : 'rechazado'} correctamente.`,
          icon: 'success',
        ...
        Swal.fire({
          ...
          text: res.error || 'No se pudo procesar la solicitud.',
          icon: 'error',
          confirmButtonColor: '#001F3F'
        })
      }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-6 md:p-12 text-[#001F3F]">
      ...
      {solicitudes.length > 0 ? (
            ...
            <div key={s.id} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden flex flex-col justify-between hover:shadow-2xl transition-all group">
              ...
              <div>
                ...
                <div className="p-8 space-y-6">
                  ...
                  <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-400 px-2 border-b border-slate-100 pb-4">
                    <span>Valor del Canje:</span>
                    <span className="text-[#001F3F] text-base font-mono font-black">${Number(s.premio.puntos).toFixed(2)}</span>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-24 text-center border-2 border-dashed border-slate-200 shadow-inner animate-fadeIn">
