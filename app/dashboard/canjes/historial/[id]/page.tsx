...
export default function DetalleCanjesVendedor({ params }: { params: Promise<{ id: string }> }) {
  ...
  const saldoDisponible = vendedor ? (Number(vendedor.puntosAcumulados) - Number(vendedor.saldoGastado)) : 0
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-6 md:p-12 text-[#001F3F]">
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-md p-6 flex flex-col sm:flex-row justify-between items-center gap-4 mb-10">
        ...
        <div className="bg-[#F8FAFC] border border-slate-100 px-6 py-2.5 rounded-2xl text-right">
          ...
          <p className="text-lg font-black text-[#FFB800] mt-1 font-mono">${saldoDisponible.toFixed(2)}</p>
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        {canjes.map((c) => {
          ...
          return (
            <div key={c.id} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden flex flex-col justify-between group h-[440px]">
              <div className="relative h-48 bg-slate-50 border-b overflow-hidden">
                <div 
              ...
              <div className="p-6 flex-grow flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-[11px] font-medium text-slate-500 line-clamp-2 italic">Valor canjeado: ${Number(c.premio.puntos).toFixed(2)} Ditcash.</p>
                </div>
