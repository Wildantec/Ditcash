      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10">
        ...
        <div className="lg:col-span-4 bg-white rounded-[2.5rem] p-6 md:p-8 shadow-xl border border-slate-100 h-fit">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button 
              disabled={loading} 
              className="w-full bg-[#001F3F] text-[#FFB800] py-5 rounded-2xl font-black text-[11px] uppercase tracking-[0.3em] shadow-lg active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
              ...
              {evidencias.map((ev: any) => (
                <div key={ev.id} className="bg-white rounded-[2.5rem] overflow-hidden shadow-md border border-slate-100 flex flex-col group hover:shadow-xl transition-all">
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    ...
                    {ev.estado === 'rechazado' && ev.motivoRechazo && (
                      <div className="bg-red-50 border border-red-100 p-3 rounded-xl mb-4">
                        <p className="text-[7px] font-black uppercase text-red-400">Motivo del Rechazo:</p>
                        <p className="text-[9px] text-red-600 font-bold italic uppercase mt-0.5 line-clamp-2 leading-tight">
                          {ev.motivoRechazo}
                        </p>
                      </div>
                    )}
