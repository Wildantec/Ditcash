...
export default function Home() {
  ...
  const manejarFlujoPrincipal = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    setClienteNombre(''); // Crucial: limpia el nombre anterior antes de buscar el nuevo
    ...
    try {
      ...
      alert("Error de comunicación. Revise los logs de Docker.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const manejarActivacionClave = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Enviamos cédula y la nueva contraseña para CREAR el usuario
      ...
      alert("No se pudo establecer la contraseña.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex flex-col min-h-screen bg-[#F4F7FA]">
            ...
            <div className="bg-white rounded-[2.5rem] shadow-[0_40px_80px_rgba(0,31,63,0.08)] border border-white overflow-hidden relative">
                  ...
                  <>
                    ...
                    <div className="p-8 md:p-10">
                      <form onSubmit={manejarFlujoPrincipal} className="space-y-5">
                        ...
                        <button type="submit" disabled={loading} className="w-full py-5 rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl bg-[#001F3F] text-[#FFB800]">
                ...
                {step === 'confirm_activation' && (
                  <div className="p-10 text-center space-y-6">
                    <div className="w-16 h-16 bg-[#FFB800]/10 rounded-full flex items-center justify-center mx-auto text-2xl">👋</div>
                    <h3 className="text-xl font-black text-[#001F3F] uppercase italic">¡Hola, {clienteNombre}!</h3>
                    <p className="text-slate-500 text-sm font-medium">Hemos detectado que eres cliente de Wildantec. ¿Deseas activar tu cuenta web ahora?</p>
                    <div className="grid grid-cols-2 gap-4">
                        <button onClick={() => setStep('formulario')} className="py-3 bg-slate-50 text-slate-400 rounded-xl font-black uppercase text-[10px]">No, luego</button>
                        <button onClick={() => setStep('change_password')} className="py-3 bg-[#001F3F] text-[#FFB800] rounded-xl font-black uppercase text-[10px]">Sí, Activar</button>
                    </div>
                  </div>
                )}
                ...
                {step === 'change_password' && (
                  <div className="p-10 text-left">
                    <form onSubmit={manejarActivacionClave} className="space-y-5">
                      <h3 className="text-lg font-black text-[#001F3F] uppercase italic">Establecer Nueva Clave</h3>
                      <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Nueva Contraseña</label>
                          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#FFB800] text-lg font-bold text-[#001F3F]" required />
                      </div>
                      <button type="submit" disabled={loading} className="w-full py-5 rounded-2xl font-black uppercase tracking-[0.2em] bg-[#001F3F] text-[#FFB800] shadow-xl">
                          Guardar e Ingresar ➔
                      </button>
                    </form>
                  </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-[#001F3F]/90 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-8 md:p-10 relative shadow-2xl">
            ...
            <div className="grid grid-cols-1 gap-3 mt-8">
                ...
                <Link href="/politica-privacidad" className="text-center text-[9px] font-black text-slate-400 uppercase tracking-widest py-2">Ver documento completo ➔</Link>
            </div>
          </div>
        </div>
      )}
